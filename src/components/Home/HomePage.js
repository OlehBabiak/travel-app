...
const HomePage=() => {
    ...
    const from = selectedByDuration.length === 0? 0: +selectedByDuration.split('_x_')[0];
    const to = selectedByDuration.length === 0? 1000: +selectedByDuration.split('_x_')[1]

    const sortedArray = AVAILABLE_TRIPS
        .filter(trip=> {
            return trip['title'].toLowerCase().includes(searchValue.toLowerCase().trim())
        })
        .filter(trip=> {return trip['duration'] >= from && trip['duration'] < to})
        .filter(trip => {
